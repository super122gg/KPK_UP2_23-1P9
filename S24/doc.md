...
- Блокировки со статусом `cancelled` (status_id = 2) **не участвуют** в проверке пересечения интервалов (реализовано в `logic.py`, константа `CANCELLED_STATUS_ID = 2` в `models.py`).
