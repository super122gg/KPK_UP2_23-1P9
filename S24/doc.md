...
- `is_active` по умолчанию `true` (активная запись), при мягком удалении устанавливается `false`.
...
- `(room_id, start_datetime, end_datetime)` уникальна (проверяется на уровне бизнес-логики с учётом `is_active = true`).
...
Если запись помечена как удалённая (`is_active = false`), возвращается `404 Not Found`.
...
| `date_from` | Левая граница периода | Нет | DateTime | ISO 8601. Ищутся блокировки, у которых `end_datetime > date_from` |
| `date_to` | Правая граница периода | Нет | DateTime | ISO 8601. Ищутся блокировки, у которых `start_datetime < date_to` |
...
**Возвращаемый список** – массив объектов с теми же полями, что и при создании (только записи с `is_active = true`).
