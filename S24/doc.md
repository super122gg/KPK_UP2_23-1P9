...
- `(room_id, start_datetime, end_datetime)` уникальна для активных записей (проверяется на уровне бизнес-логики).
...
**Возвращаемый список** – массив объектов:
| Параметр | Тип |
|---|---|
| `id` | Integer |
| `name` | String |
...
| Параметр | Пояснение | Обязательность | Тип | Ограничение |
|---|---|---|---|---|
| `status_id` | ID статуса | Да | Integer | > 0 |

**Успешный ответ:** `200 OK`

**Возвращаемые данные** – объект с полями `id`, `name`, `description`.
...
| `GET` | `/blocks/` | Получить список блокировок (включая удалённые) |
| `GET` | `/statuses/` | Получить список статусов |
| `GET` | `/statuses/{status_id}` | Получить статус по ID |
